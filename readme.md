...
health_backend：美年健康管理后台，打包方式为war，作为Dubbo服务消费方，存放Controller、HTML页面、js、css、spring配置文件等，需要部署到tomcat运行

health_mobile：移动端前台，打包方式为war，作为Dubbo服务消费方，存放Controller、HTML页面、js、css、spring配置文件等，需要部署到tomcat运行

### 2.2 maven项目搭建

通过前面的项目功能架构图可以知道本项目分为美年健康管理后台和美年健康前台（微信端）
...
创建health_service_provider，webapp子工程，打包方式为war，作为服务单独部署，存放服务类、Dao接口和Mapper映射文件等
...
db.properties（mysql5）

```
jdbc.driverClassName=com.mysql.jdbc.Driver
jdbc.url=jdbc:mysql://localhost:3306/health?characterEncoding=utf8
jdbc.username=root
jdbc.password=root
```

db.properties（mysql8）

```
jdbc.driverClassName=com.mysql.cj.jdbc.Driver
jdbc.url=jdbc:mysql://localhost:3306/health?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai&useSSL=false
jdbc.username=root
jdbc.password=root
```
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <bean id="sqlSessionFactory" class="org.mybatis.spring.SqlSessionFactoryBean">
        ...
        <!--别名-->
        <property name="typeAliasesPackage" value="com.bigdata.pojo"></property>
        <!--mapper文件位置-->
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <!--批量扫描，发布服务 新建com.bigdata.service-->
    <dubbo:annotation package="com.bigdata.service"/>
</beans>
```
...
创建health_backend，webapp子工程，打包方式为war，单独部署，存放Controller、页面等
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <!--指定服务注册中心地址-->
    <dubbo:registry address="zookeeper://127.0.0.1:2181"/>
    <!--批量扫描 新建com.bigdata.controller-->
    <dubbo:annotation package="com.bigdata.controller" />
    <!--
        超时全局设置 10分钟
...
项目install，运行health_backend、health_service_provider
...
美年健康项目后台系统就是使用ElementUI来构建页面，在页面上引入 js 和 css 文件即可开始使用，如下：
